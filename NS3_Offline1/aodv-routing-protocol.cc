...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        packet->AddHeader(rreqHeader);
        TypeHeader tHeader(AODVTYPE_RREQ);/////////////////change////////
        packet->AddHeader(tHeader);
        // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
        Ipv4Address destination;
        if (iface.GetMask() == Ipv4Mask::GetOnes())
...
void
RoutingProtocol::RecvReverseRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);
    ...
    if (ShouldIgnoreRequest(src, revreqHeader)) {
        return;
    }

    uint32_t id = revreqHeader.GetId();
    Ipv4Address origin = revreqHeader.GetOrigin();
    ...
    RoutingTableEntry toDst;
    if (!UpdateOrCreateRoute(receiver, src, revreqHeader, toDst)) {
        return;
    }
    ...
    {////////////////////////////////////
        m_routingTable.LookupRoute(revreqHeader.GetDst(), toDst);
        NS_LOG_DEBUG("Send reply since I am the destination");
        SendPacketFromQueue(revreqHeader.GetDst(), toDst.GetRoute()); 
        return;
    }
    /*
     * (ii) or it has an active route to the destination, the destination sequence number in the
    ...
    if (!m_routingTable.LookupRoute(dst, toOrigin)||toOrigin.GetFlag() == IN_SEARCH)
    {
        /*
         * Drop RREQ, This node RREP will make a loop.
         */
    ...
    if (tag.GetTtl() < 2)
    {
     ...
     BroadcastRreq(revreqHeader);
   
}


bool ns3::aodv::RoutingProtocol::ShouldIgnoreRequest(ns3::Ipv4Address src, ns3::aodv::RevRreqHeader &revreqHeader) {
    RoutingTableEntry toPrev;
...
bool ns3::aodv::RoutingProtocol::UpdateOrCreateRoute(ns3::Ipv4Address receiver,ns3:: Ipv4Address src,
                                        ns3::aodv::  RevRreqHeader &revreqHeader,ns3::aodv:: RoutingTableEntry &toDst) {
    if (!m_routingTable.LookupRoute(revreqHeader.GetDst(), toDst)) {
        AddNewRoute(receiver, src, revreqHeader);
        return true;
    }

    UpdateExistingRoute(src, receiver, revreqHeader, toDst);
    return true;
}
void ns3::aodv:: RoutingProtocol::AddNewRoute(ns3::Ipv4Address receiver,ns3::Ipv4Address src,ns3::aodv:: RevRreqHeader &revreqHeader) {
    Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
    RoutingTableEntry newEntry(
        dev, revreqHeader.GetDst(), true, revreqHeader.GetOriginSeqno(),
        m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0), revreqHeader.GetHopCount(),
        src, Time(2 * m_netTraversalTime - 2 * revreqHeader.GetHopCount() * m_nodeTraversalTime));
    m_routingTable.AddRoute(newEntry);
}


void ns3:: aodv::RoutingProtocol::UpdateExistingRoute(ns3::Ipv4Address src,ns3::Ipv4Address receiver,
                                         ns3::aodv:: RevRreqHeader &revreqHeader,ns3::aodv:: RoutingTableEntry &toDst) {
    if (toDst.GetValidSeqNo() &&
        int32_t(revreqHeader.GetOriginSeqno()) > int32_t(toDst.GetSeqNo())) {
        toDst.SetSeqNo(revreqHeader.GetOriginSeqno());
    }
    ...
    toDst.SetHop(revreqHeader.GetHopCount());
    toDst.SetLifeTime(std::max(Time(2 * m_netTraversalTime - 2 * revreqHeader.GetHopCount() *
                                   m_nodeTraversalTime),
                               toDst.GetLifeTime()));
    m_routingTable.Update(toDst);
}
...
void ns3::aodv:: RoutingProtocol::BroadcastRreq(ns3::aodv:: RevRreqHeader &revreqHeader) {
    for (auto &socketAddress : m_socketAddresses) {
        ...
        TypeHeader tHeader(AODVTYPE_REV_RERQ);
        packet->AddHeader(tHeader);
...
RoutingProtocol::RevSendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
   ...
   RevRreqHeader Rev_RrepHeader(0,0,0,rreqHeader.GetId(),rreqHeader.GetDst(),m_seqNo,rreqHeader.GetOrigin(),0);
 //RevRreqHeader Rev_RrepHeader(0,0,0,rreqHeader.GetId(),rreqHeader.GetDst(),m_seqNo,toOrigin.GetDestination(),0);
    TypeHeader tHeader(AODVTYPE_REV_RERQ);
    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(Rev_RrepHeader);
    packet->AddHeader(tHeader);
    for (const auto& socketAddress : m_socketAddresses)
    {
        ...
        Ipv4Address destination = iface.GetMask()==(Ipv4Mask::GetOnes()) 
                                  ...
                                  : iface.GetBroadcast();
        NS_LOG_DEBUG("Broadcasting Rev_RREP with ID " << Rev_RrepHeader.GetId() << " to " << destination);

        // Clone the packet to send individually on each interface
        Ptr<Packet> packetCopy = packet->Copy();
        // Add TTL tag to the packet copy
        SocketIpTtlTag tag;
        tag.SetTtl(toOrigin.GetHop());
        ...
        Simulator::Schedule(
            ...
            &RoutingProtocol::SendTo, this, socket, packetCopy, destination);
    }
}
