...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("nodeNumber", "Number of nodes", m_nodeNumber);  // New argument to take node count
    cmd.AddValue("packet_Number","Number of packets",m_packetNumber);
    cmd.AddValue("nodeSpeed", "Number of packets per second", m_nodespeed);
    cmd.Parse(argc, argv);
...
RoutingExperiment::Run()
{
   ...
   int nWifis=m_nodeNumber;
   int packet=m_packetNumber;
   m_nSinks=m_nSinks/2;
   int new_node_speed=m_nodespeed;
   std::string packetNumber=std::to_string(m_packetNumber*64*8)+"bps";
//    std::
//    std::string rate("2048bps");

   std::cout<<nWifis<<std::endl;
    double TotalTime = 150.0;
    std::string rate(packetNumber);
    std::string phyMode("DsssRate11Mbps");
    std::string tr_name("manet-routing-compare");
    int nodeSpeed = new_node_speed; // in m/s
    ...
    Simulator::Run();
    std::cout<<nWifis<<std::endl;
    std::cout<<nodeSpeed<<std::endl;
    std::cout<<packetNumber<<std::endl;
    ...
    if (m_flowMonitor)
{
    ...
    // Extract FlowMonitor statistics
    ...
    {
        // Get flow classifier details
        Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow(flow.first);

        // Aggregate metrics
        totalBytesReceived += flow.second.rxBytes;
        totalPacketsSent += flow.second.txPackets;
    ...
    // Calculate Metrics
    double throughput = (totalBytesReceived * 8) / simulationTime; 
    double avgEndToEndDelay = totalPacketsReceived > 0 ? (totalDelay / totalPacketsReceived) : 0;
    double packetDeliveryRatio = (totalPacketsSent > 0) ? (totalPacketsReceived / totalPacketsSent) * 100 : 0;
    double packetDropRatio = (totalPacketsSent > 0) ? (totalPacketsDropped / totalPacketsSent) * 100 : 0;

    // Output Metrics
    std::cout << "Network Metrics:" << std::endl;
    std::cout << "1. Throughput: " << throughput << " bps" << std::endl;
    std::cout << "2. Average End-to-End Delay: " << avgEndToEndDelay << " seconds" << std::endl;
    std::cout << "3. Packet Delivery Ratio: " << packetDeliveryRatio << " %" << std::endl;
    std::cout << "4. Packet Drop Ratio: " << packetDropRatio << " %" << std::endl;

    std::ofstream metricsFile("Task2.csv", std::ios::app);
        metricsFile << nWifis << "," << packet << "," << nodeSpeed << ","
                    << (throughput/1024) << "," << avgEndToEndDelay << "," << packetDeliveryRatio << "," << packetDropRatio << "\n";
        metricsFile.close();
}
